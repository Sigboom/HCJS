		...
		<ul class="col-sm-12 followersList">
			...
			<li v-for="(follower, index) in this.$store.state.followerList" :key="index" class="col-md-12">
				<div class="followerListPot col-xs-12" @click="askForTalk(index, follower)" role="button">
					<a role="button" class="btn btn-default col-md-2"
					@click="addUser(follower, index)" v-show="!follower.inTeam">
					...
					<span class="col-md-6">
						<p class="col-md-12">{{ follower.name }}</p>
...
export default {
	...
	methods: {	
		addUser(follower, index) {
			console.log(follower);
			this.$store.commit('addFollowerToTeam', follower);
			console.log("add OK");
		...
		getFollowerList() {
			if (this.$store.state.userID) {
				this.$axios.get('/getFollower', {
					...
					if (followerList.length != this.$store.state.followerCounter) {
